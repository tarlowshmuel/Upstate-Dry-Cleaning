...
**Phase 1 day-picker (added later):** Phase 1 towns are now bookable on any Mon–Thu — customers choose via a `pickup_day` step. The `pickup`/`dropoff` weekday fields on Phase 1 schedule entries are vestigial; `nextPickupOptions(town, count, now)` is the source of truth and ignores them for Phase 1. The per-town `wave` (morning/afternoon) still applies and gates the same-day cutoff (10 AM / 12 PM) on whichever day they pick. Phase 2 entries keep fixed-weekday semantics for the day Phase 2 ships. Never read `schedule.pickup` for Phase 1 logic.
